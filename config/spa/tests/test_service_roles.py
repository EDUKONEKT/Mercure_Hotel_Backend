...
def sample_spa():
    ...
    return Spa.objects.create(price=50.0, is_available=True, type="spa")

@pytest.fixture
def booking_url():
    return "/api/spa/spa-bookings/"
...
def test_service_permissions(api_client, role, method, expected_status, service_url):
    ...
    data = {"price": 99.0, "is_available": True, "type": "spa"}

    # Exécute la bonne méthode HTTP
    func = getattr(api_client, method)
    if method == "put":
        # on crée un service d'abord, puis on le modifie
        spa = Spa.objects.create(price=50.0, is_available=True, type="spa")
        url = f"{service_url}{spa.id}/"
        response = func(url, {"price": 120.0, "is_available": True,"type":"spa"}, content_type='application/json', **headers)
    elif method == "delete":
        spa = Spa.objects.create(price=50.0, is_available=True, type="spa")
        url = f"{service_url}{spa.id}/"
...
def test_anyone_can_list_services(api_client, service_url):
    ...
    Spa.objects.create(price=40.0, is_available=True, type="spa")
...
def test_create_booking_by_role(api_client, sample_spa, role, expected_status, booking_url):
    """Vérifie qui peut créer une réservation"""
    user = create_user_with_role(f"user_{role}", role)
    headers = get_auth_headers(user)
    data = {"spa": sample_spa.id, "check_in": "2025-11-01", "check_out": "2025-11-02"}
...
def test_delete_booking_permissions(api_client, sample_spa, role, expected_status, booking_url):
    ...
    booking = SpaBooking.objects.create(spa=sample_spa, account=owner_account,
...
@pytest.mark.django_db
def test_receptionist_can_create_booking_for_client(api_client, sample_spa, booking_url):
    ...
    assert  float(response.data["total_price"]) == sample_spa.price * 2
