...
def sample_meal():
    ...
    return Meal.objects.create(price=50.0, is_available=True, type="restaurant", meal_type="lunch")

@pytest.fixture
def booking_url():
    return "/api/restaurant/meal-bookings/"

@pytest.fixture
def service_url():
    return "/api/restaurant/meals/"
...
def test_service_permissions(api_client, role, method, expected_status, service_url):
    ...
    data = {"price": 99.0, "is_available": True, "type":"restaurant", "meal_type":"lunch"}

    # Exécute la bonne méthode HTTP
    func = getattr(api_client, method)
    if method == "put":
        # on crée un service d'abord, puis on le modifie
        meal = Meal.objects.create(price=50.0, is_available=True, type="restaurant", meal_type="lunch")
        url = f"{service_url}{meal.id}/"
        response = func(url, {"price": 120.0, "is_available": True, "type":"restaurant", "meal_type":"lunch"}, content_type='application/json', **headers)
    elif method == "delete":
        meal = Meal.objects.create(price=50.0, is_available=True, type="restaurant", meal_type="lunch")
        url = f"{service_url}{meal.id}/"
...
def test_anyone_can_list_services(api_client, service_url):
    ...
    Meal.objects.create(price=40.0, is_available=True, type="restaurant", meal_type="lunch")
...
def test_create_booking_by_role(api_client, sample_meal, role, expected_status, booking_url):
...
def test_delete_booking_permissions(api_client, sample_meal, role, expected_status, booking_url):
...
@pytest.mark.django_db
def test_receptionist_can_create_booking_for_client(api_client, sample_meal, booking_url):
    ...
    assert  float(response.data["total_price"]) == sample_meal.price * 2
