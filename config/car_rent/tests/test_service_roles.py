...
def sample_car():
    ...
    return Car_rent.objects.create(price=50.0, is_available=True, type="car_rent",name="RENAULT CAPTUR",qty=3)

@pytest.fixture
def booking_url():
    return "/api/car_rent/car_rent_bookings/"

@pytest.fixture
def service_url():
    return "/api/car_rent/car_rent/"
...
def test_service_permissions(api_client, role, method, expected_status, service_url):
    ...
    data = {"price": 99.0, "is_available": True, "type": "car_rent", "name":"RENAULT CAPTUR", "qty": 2}

    # Exécute la bonne méthode HTTP
    func = getattr(api_client, method)
    if method == "put":
        # on crée un service d'abord, puis on le modifie
        car_rent = Car_rent.objects.create(price=50.0, is_available=True, type="car_rent",name="RENAULT CAPTUR",qty=3)
        url = f"{service_url}{car_rent.id}/"
        response = func(url, {"price": 120.0, "is_available": True,"type":"car_rent", "name":"RENAULT CAPTUR", "qty":3}, content_type='application/json', **headers)
    elif method == "delete":
        car = Car_rent.objects.create(price=50.0, is_available=True, type="car_rent",name="RENAULT CAPTUR",qty=3)
        url = f"{service_url}{car.id}/"
...
def test_anyone_can_list_services(api_client, service_url):
    ...
    Car_rent.objects.create(price=40.0, is_available=True, type="car_rent",name="car",qty=3)
...
def test_create_booking_by_role(api_client, sample_car, role, expected_status, booking_url):
    """Vérifie qui peut créer une réservation"""
    user = create_user_with_role(f"user_{role}", role)
    headers = get_auth_headers(user)
    data = {"car_rent": sample_car.id, "check_in": "2025-11-01", "check_out": "2025-11-02"}
...
def test_delete_booking_permissions(api_client, sample_car, role, expected_status, booking_url):
    ...
    booking = CarRentBooking.objects.create(car_rent=sample_car, account=owner_account,
...
@pytest.mark.django_db
def test_receptionist_can_create_booking_for_client(api_client, sample_car, booking_url):
    ...
    assert  float(response.data["total_price"]) == sample_car.price * 2
