...
def sample_sauna():
    ...
    return Sauna.objects.create(price=50.0, is_available=True, type="sauna")

@pytest.fixture
def booking_url():
    return "/api/sauna/sauna-bookings/"

@pytest.fixture
def service_url():
    return "/api/sauna/saunas/"
...
def test_service_permissions(api_client, role, method, expected_status, service_url):
    ...
    data = {"price": 99.0, "is_available": True, "type": "sauna"}

    # Exécute la bonne méthode HTTP
    func = getattr(api_client, method)
    if method == "put":
        # on crée un service d'abord, puis on le modifie
        sauna = Sauna.objects.create(price=50.0, is_available=True, type="sauna")
        url = f"{service_url}{sauna.id}/"
        response = func(url, {"price": 120.0, "is_available": True,"type":"sauna"}, content_type='application/json', **headers)
    elif method == "delete":
        sauna = Sauna.objects.create(price=50.0, is_available=True, type="sauna")
        url = f"{service_url}{sauna.id}/"
...
def test_anyone_can_list_services(api_client, service_url):
    ...
    Sauna.objects.create(price=40.0, is_available=True, type="sauna")
...
def test_create_booking_by_role(api_client, sample_sauna, role, expected_status, booking_url):
    """Vérifie qui peut créer une réservation"""
    user = create_user_with_role(f"user_{role}", role)
    headers = get_auth_headers(user)
    data = {"sauna": sample_sauna.id, "check_in": "2025-11-01", "check_out": "2025-11-02"}
...
def test_delete_booking_permissions(api_client, sample_sauna, role, expected_status, booking_url):
    ...
    booking = SaunaBooking.objects.create(sauna=sample_sauna, account=owner_account,
...
@pytest.mark.django_db
def test_receptionist_can_create_booking_for_client(api_client, sample_sauna, booking_url):
    ...
    assert  float(response.data["total_price"]) == sample_sauna.price * 2
