...
def sample_room():
    ...
    return Room.objects.create(price=50.0, is_available=True, type="rooms", number=102, name="Single")

@pytest.fixture
def booking_url():
    return "/api/rooms/room-bookings/"

@pytest.fixture
def service_url():
    return "/api/rooms/rooms/"
...
def test_service_permissions(api_client, role, method, expected_status, service_url):
    ...
    data = {"price": 99.0, "is_available": True, "type":"rooms", "number":102, "name":"Single"}

    # Exécute la bonne méthode HTTP
    func = getattr(api_client, method)
    if method == "put":
        # on crée un service d'abord, puis on le modifie
        room = Room.objects.create(price=50.0, is_available=True,  type="rooms", number=102, name="Single")
        url = f"{service_url}{room.id}/"
        response = func(url, {"price": 120.0, "is_available": True,"type":"rooms", "number":102, "name":"Single"}, content_type='application/json', **headers)
    elif method == "delete":
        room = Room.objects.create(price=50.0, is_available=True, type="rooms", number=102, name="Single")
        url = f"{service_url}{room.id}/"
...
def test_anyone_can_list_services(api_client, service_url):
    ...
    Room.objects.create(price=40.0, is_available=True, type="rooms", number=102, name="SINGLE")
...
def test_create_booking_by_role(api_client, sample_room, role, expected_status, booking_url):
    """Vérifie qui peut créer une réservation"""
    user = create_user_with_role(f"user_{role}", role)
    headers = get_auth_headers(user)
    data = {"room": sample_room.id, "check_in": "2025-11-01", "check_out": "2025-11-02"}
...
def test_delete_booking_permissions(api_client, sample_room, role, expected_status, booking_url):
    ...
    booking = RoomBooking.objects.create(room=sample_room, account=owner_account,
...
@pytest.mark.django_db
def test_receptionist_can_create_booking_for_client(api_client, sample_room, booking_url):
    ...
    assert  float(response.data["total_price"]) == sample_room.price * 2
